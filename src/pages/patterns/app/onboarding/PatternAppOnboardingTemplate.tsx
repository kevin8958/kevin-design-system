...
import AppOnboardingPreview from '@/pages/patterns/app/onboarding/AppOnboardingPreview';
...
export default function PatternAppOnboardingTemplate({
  config,
}: {
  config: OnboardingPatternConfig;
}) {
  const [state, setState] = useState<OnboardingPreviewState>('default');
  const [mode, setMode] = useState<OnboardingPreviewMode>('standard');
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={state}
                onChange={(next) =>
                  setState(next as OnboardingPreviewState)
                }
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={mode}
                onChange={(next) => setMode(next as OnboardingPreviewMode)}
              />
            </div>
          </div>
        </FlexWrapper>
          ...
          example={
            <CodeExample code={guide.code} className="w-full">
              <AppDevicePreviewFrame
                minHeight={config.appPreviewMinHeight ?? 720}
                maxWidthClass="max-w-[430px]"
              >
                <AppOnboardingPreview
                  patternId={config.id}
