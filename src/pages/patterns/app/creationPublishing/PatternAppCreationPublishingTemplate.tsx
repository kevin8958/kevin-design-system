...
import AppCreationPublishingPreview from '@/pages/patterns/app/creationPublishing/AppCreationPublishingPreview';
...
export default function PatternAppCreationPublishingTemplate({
  config,
}: {
  config: CreationPublishingPatternConfig;
}) {
  const [state, setState] = useState<CreationPublishingPreviewState>('default');
  const [mode, setMode] = useState<CreationPublishingPreviewMode>('standard');

  return (
    <PatternDocsPageShell
      platform="app"
      categoryId="creation-publishing"
      categoryLabel="Creation / Publishing"
      patternId={config.id}
      title={config.title}
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={state}
                onChange={(next) =>
                  setState(next as CreationPublishingPreviewState)
                }
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={mode}
                onChange={(next) =>
                  setMode(next as CreationPublishingPreviewMode)
                }
              />
            </div>
          </div>
