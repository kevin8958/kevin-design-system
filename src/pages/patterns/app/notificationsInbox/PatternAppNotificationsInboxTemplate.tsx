...
import AppNotificationsInboxPreview from '@/pages/patterns/app/notificationsInbox/AppNotificationsInboxPreview';
...
export default function PatternAppNotificationsInboxTemplate({
  config,
}: {
  config: NotificationsInboxPatternConfig;
}) {
  const [state, setState] = useState<NotificationsInboxPreviewState>('default');
  const [mode, setMode] = useState<NotificationsInboxPreviewMode>('standard');

  return (
    <PatternDocsPageShell
      platform="app"
      categoryId="notifications-inbox"
      categoryLabel="Notifications / Inbox"
      patternId={config.id}
      title={config.title}
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={state}
                onChange={(next) =>
                  setState(next as NotificationsInboxPreviewState)
                }
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={mode}
                onChange={(next) =>
                  setMode(next as NotificationsInboxPreviewMode)
                }
              />
            </div>
          </div>
