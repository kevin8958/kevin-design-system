...
import WebCommercePlansPreview from '@/pages/patterns/web/commercePlans/WebCommercePlansPreview';
...
export default function PatternWebCommercePlansTemplate({
  config,
}: {
  config: CommercePlansPatternConfig;
}) {
  const [state, setState] = useState<CommercePlansPreviewState>('default');
  const [mode, setMode] = useState<CommercePlansPreviewMode>('standard');

  return (
    <PatternDocsPageShell
      platform="web"
      categoryId="commerce-plans"
      categoryLabel="Commerce / Plans"
      patternId={config.id}
      title={config.title}
          ...
          <div className="flex w-full flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={state}
                onChange={(next) =>
                  setState(next as CommercePlansPreviewState)
                }
              />
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <ButtonGroup
                ...
                value={mode}
                onChange={(next) =>
                  setMode(next as CommercePlansPreviewMode)
                }
              />
            </div>
          </div>
